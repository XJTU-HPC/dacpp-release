...
const size_t N = 10;
const size_t K = 10;
const size_t TILE_SIZE = 16; // Local workgroup tile size (aligned with the first code segment)
...
void matrix_multiply_sycl(
    ...
    //std::cout << "Using local workgroup size: " << local_x << "x" << local_y << std::endl;

    // 2. Calculate aligned global work item size (must be an integer multiple of local size, consistent with the first code segment)
    ...
    // 3. Create 2D SYCL Buffer (bind host-side vector data)
    ...
    // 4. Submit SYCL command group (use nd_range to implement global/local optimization)
    q.submit([&](sycl::handler& h) {
        // 4.1 Create Accessor (device-side interface for accessing Buffer)
        auto acc_A = buf_A.get_access<sycl::access::mode::read>(h);
        auto acc_B = buf_B.get_access<sycl::access::mode::read>(h);
        auto acc_C = buf_C.get_access<sycl::access::mode::write>(h);

        // 4.2 Define nd_range (global range + local workgroup range)
        ...
        // 4.3 Parallel kernel function (using nd_item<2>, supports global/local indexing)
        h.parallel_for(nd_range, [=](sycl::nd_item<2> item) {
            // Get global index (corresponds to row i, column j of matrix C)
            size_t i = item.get_global_id(0);
            size_t j = item.get_global_id(1);
            
            if (i < M && j < N) {
                int sum = 0;
                // Compute the dot product sum for row i, column j (keep original logic, can further combine with local memory optimization later)
...
int main() {
    ...
    std::vector<int> dataC(M * N, 0);  // C is M rows by N columns, initially all 0
    ...
    // std::cout << "Total running time: " << total_program << " seconds" << std::endl;
