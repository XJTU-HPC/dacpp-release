...
void run(std::vector<int>& array) {
    ...
    dacpp::Tensor<int, 1> wrong_slice = array_out_tensor[{1, N - 1}];
    std::vector<int> wrong_out(N - 2, 0);
    dacpp::Tensor<int, 1> wrong_out_tensor(wrong_out);
    ...
    for (int phase = 0; phase < 2; phase++) {
        ...
        ODDEVEN(wrong_slice, wrong_out_tensor) <-> oddeven;
